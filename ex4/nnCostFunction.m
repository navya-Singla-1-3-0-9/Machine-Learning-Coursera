...
J = (1/m)*(sum)(sum((-y2).*(log(h))-(1-y2).*log(1-h))) + (lambda/(2*m))*(sum(sum(t1.^2))+sum(sum(t2.^2)));
...
for t=1:m
  ...
  delta_3 = a3 - y2(t,:)'; % (10*1)
  mat= [zeros(1);sigmoidGradient(z2)];
   delta_2 = (Theta2' * delta_3) .* mat; % ((26*10)*(10*1))=(26*1)
